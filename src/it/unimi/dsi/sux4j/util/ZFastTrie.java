 ...
 *
 * Copyright (C) 2010-2018 Sebastiano Vigna
 *
...
public class ZFastTrie<T> extends AbstractObjectSortedSet<T> implements Serializable {
	...
	public ParexData<T> getParentExitNode(final LongArrayBitVector v, final long[] state, final ObjectArrayList<InternalNode<T>> stack) {
		...
		if (parexOrExitNode.isExitNodeOf(length, lcpLength, transform)) {
			...
			return new ParexData<>(fatBinarySearch(v, state, stack, true, startingPoint, parexOrExitNode.nameLength), parexOrExitNode, lcpLength);
		}
		...
		return new ParexData<>(fatBinarySearch(v, state, stack, true, startingPoint, parexOrExitNode.nameLength), parexOrExitNode, lcpLength);  // TODO check -1
