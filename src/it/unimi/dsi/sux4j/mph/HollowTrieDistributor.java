...
public class HollowTrieDistributor<T> extends AbstractObject2LongFunction<T> {
	...
	/** The number of internal node of the trie. */
	private final int size;
	/** A debug function used to store explicitly {@link #internalBehaviour}. */
	private transient Object2LongFunction<BitVector> internalTestFunction;
	/** A debug function used to store explicitly {@link #externalBehaviour}. */
	private transient Object2LongFunction<BitVector> externalTestFunction;
	
	/** An intermediate class containing the compacted trie generated by the delimiters. After its construction,
	 * {@link #internalKeys} and {@link #externalKeys} contain the pairs node/path that must be mapped
	 * to {@link #values} and {@link #lValues}, respectively, to obtain the desired behaviour. */
	private final static class IntermediateTrie<T> {
		...
		private File internalKeys;
		private File externalKeys;
		...
		public IntermediateTrie( final Iterable<? extends T> elements, final int bucketSize, final TransformationStrategy<? super T> transformationStrategy, final File tempDir ) throws IOException {
			...
			if ( iterator.hasNext() ) {
				...
				if ( root != null ) {
					...
					internalKeys = File.createTempFile( HollowTrieDistributor.class.getName(), "int", tempDir );
					externalKeys = File.createTempFile( HollowTrieDistributor.class.getName(), "ext", tempDir );
					internalKeys.deleteOnExit();
					externalKeys.deleteOnExit();
					OutputBitStream keys = new OutputBitStream( internalKeys );
					OutputBitStream lkeys = new OutputBitStream( externalKeys );
							...
							if ( prefix < nodePath.length() || ! node.seen ) {
								...
								if ( lastNode != node || ! path.equals( lastPath ) ) {
									...
									if ( ASSERTS ) {
										...
										if ( node.isLeaf() ) externalTestFunction.put( LongArrayBitVector.wrap( key, pathLength + Long.SIZE ), value );
										else internalTestFunction.put( LongArrayBitVector.wrap( key, pathLength + Long.SIZE ), value );
									}
	...
	public HollowTrieDistributor( final Iterable<? extends T> elements, final int bucketSize, final TransformationStrategy<? super T> transformationStrategy, final File tempDir ) throws IOException {
		...
		final IntermediateTrie<T> immutableBinaryTrie = new IntermediateTrie<T>( elements, bucketSize, transformationStrategy, tempDir );
		...
		if ( immutableBinaryTrie.root != null ) {
			...
			internalTestFunction = immutableBinaryTrie.internalTestFunction;
			externalTestFunction = immutableBinaryTrie.externalTestFunction;
		...
		internalBehaviour = new MWHCFunction<BitVector>( new IterableStream( new InputBitStream( immutableBinaryTrie.internalKeys ), numKeys, internalTestFunction, immutableBinaryTrie.values ) ,TransformationStrategies.identity(), immutableBinaryTrie.values, 2 );
		externalBehaviour = new MWHCFunction<BitVector>( new IterableStream( new InputBitStream( immutableBinaryTrie.externalKeys ), numLKeys, externalTestFunction, immutableBinaryTrie.lvalues ) ,TransformationStrategies.identity(), immutableBinaryTrie.lvalues, 1 );

		immutableBinaryTrie.internalKeys.delete();
		immutableBinaryTrie.externalKeys.delete();
	...
	public long getLong( final Object o ) {
		...
		for(;;) {
			...
			exit = isInternal ? (int)internalBehaviour.getLong( key.length( 0 ).append( p, Long.SIZE ).append( bitVector.subVector( s, Math.min( length, s + skip ) ) ) )
