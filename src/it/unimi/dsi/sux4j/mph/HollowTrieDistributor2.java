...
public class HollowTrieDistributor2<T> extends AbstractObject2LongFunction<T> {
				...
				if ( root != null ) {
					...
					while( iterator.hasNext() ) {
						...
						for(;;) {
							...
							if ( prefix < nodePath.length() || prefix == nodePath.length() && node.isLeaf() ) {
	...
	public HollowTrieDistributor2( final Iterable<? extends T> elements, final int bucketSize, final TransformationStrategy<? super T> transformationStrategy, final File tempDir ) throws IOException {
		...
		signatures = LongArrayBitVector.getInstance( ( size + 1 ) * 60 ).asLongBigList( 60 );
		// Turn the compacted trie into a hollow trie.
		if ( intermediateTrie.root != null ) {
			if ( DDEBUG ) System.err.println( intermediateTrie );
			visit( intermediateTrie.root, trie, 1, skips, 60, signatures );
		}
	...
	public long getLong( final Object o ) {
		...
		for(;;) {
			...
			isFollow = isInternal && signature == ( Hashes.jenkins( bitVector.subVector( s, Math.min( length, s + skip ) ) ) & ( 1 << 60 )- 1 );
	...
	public long numBits() {
		return trie.length() + skips.numBits() + signatures.length() * 3 + balParen.numBits() + externalBehaviour.numBits() + transformationStrategy.numBits();
	}
