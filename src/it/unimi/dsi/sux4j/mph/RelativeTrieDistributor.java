...
public class RelativeTrieDistributor<T> extends AbstractObject2LongFunction<T> {
	...
	private final static class IntermediateTrie<T> {
		...
		public IntermediateTrie( final Iterable<? extends T> elements, final int log2BucketSize, final TransformationStrategy<? super T> transformationStrategy ) {
	...
	public RelativeTrieDistributor( final Iterable<? extends T> elements, final int log2BucketSize, final TransformationStrategy<? super T> transformationStrategy ) {
	...
	private long getNodeStringLength( BitVector v ) {
		...
		while( r - l > 1 ) {
			...
			if ( ( l & mask ) != ( r - 1 & mask ) ) {
				...
				if ( DDDEBUG ) System.err.println( "Recalled (" + f + "," + v.subVector( 0, f ).hashCode() + ") " + v.subVector( 0, f ) + " (signature: " + ( data >>> logW ) + " length: " + ( data & logWMask ) + " data: " + data + ")" );
