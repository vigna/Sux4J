...
public class SimpleSelectZeroTest extends RankSelectTestCase {
	...
	public void testAlternating() {
		...
		assertSelectZero( select );
		for( i = 32; i-- != 1; ) assertEquals( i * 2 + 1, select.selectZero( i ) );

		select = new SimpleSelectZero( new long[] { 0x5555555555555555L, 0x5555555555555555L }, 128 );
		assertSelectZero( select );
		for( i = 64; i-- != 1; ) assertEquals( i * 2 + 1, select.selectZero( i ) );
		...
		assertSelectZero( select );
		for( i = 32 * 5; i-- != 1; ) assertEquals( i * 2 + 1, select.selectZero( i ) );

		select = new SimpleSelectZero( new long[] { 0x55555555L }, 33 );
		assertSelectZero( select );
		for( i = 16; i-- != 1; ) assertEquals( i * 2 + 1, select.selectZero( i ) );

		select = new SimpleSelectZero( new long[] { 0x5555555555555555L, 0x555555555555L }, 128 );
		assertSelectZero( select );
		for( i = 56; i-- != 1; ) assertEquals( i * 2 + 1, select.selectZero( i ) );
	}
	...
	public void testAllSizes() {
		...
		for( int size = 0; size <= 4096; size++ ) {
			...
			v.fill(  true );
			for( int i = ( size + 1 ) / 2; i-- != 0; ) v.set( i * 2, false );
			r = new SimpleSelectZero( v );
			for( int i = size / 2; i-- != 0; ) assertEquals( i * 2, r.selectZero( i ) );
			
			v = LongArrayBitVector.getInstance().length( size );
			v.fill( false );
			r = new SimpleSelectZero( v );
			for( int i = size; i-- != 0; ) assertEquals( i, r.selectZero( i ) );
		}
	}
}
