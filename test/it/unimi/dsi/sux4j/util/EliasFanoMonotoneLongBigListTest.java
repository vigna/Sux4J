...
public class EliasFanoMonotoneLongBigListTest {
	...
	private static void test(final EliasFanoMonotoneLongBigList l) throws IOException, ClassNotFoundException {
		...
		final String basename = File.createTempFile(EliasFanoMonotoneLongBigListTest.class.getSimpleName(), "test").toString();
		BinIO.storeObject(l, basename);
		assertEquals(l, BinIO.loadObject(basename));

		l.dump(basename);
		assertEquals(l, MappedEliasFanoMonotoneLongBigList.load(basename));
		new File(basename).delete();
		new File(basename + MappedEliasFanoMonotoneLongBigList.OBJECT_EXTENSION).delete();
	...
	@Test
	public void testMedium() throws ClassNotFoundException, IOException {
		LongBigArrayBigList l;
